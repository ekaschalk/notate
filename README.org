...
- Indentation masking range calculations for lisps complete I think.
...
- Generalized range of indent calculations working except when buffer
  modification support is enabled.
- Whether to mask indent calculations always depend on major mode.
