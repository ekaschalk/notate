...
- Why? :: [[http://modernemacs.com][I've found a hobby]] in confusing over-the-shoulder onlookers. I got a
          ...
          consensus on symbols and contributors to input them. I want to enable
          emoji-based programming without the typically accompanied deep shame.
...
- Buffer modification that adds or removes lines.
  - Rough solution implemented, works in most cases but won't scale.
  - Currently implementing an experimental tree-based algorithm which should
    scale to arbitrary buffer sizes/numbers of replacements in a buffer.
