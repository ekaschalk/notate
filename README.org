* nt - Always Program with Ligatures (in emacs)
...
~int~ as ~ℤ~ in their code. nt attempts to bring the joy and readability
...
*nt visually-only replaces strings with other strings, masking indentation*.
Our working indentation is visually modified so that in the example above, we
see example 3 while the true text is example 1.
...
to "prettified-symbols" and "ligatures" interchangeably. /As far as this work is
...
- Does nt support all programming languages? :: Yes-ish.
  - /Details/: nt is major-mode agnostic except for a fn to calculate the
...
- How does it work? :: By (ab)using particular properties of emacs's ~display~
  ...
  - /Details/: My approach [[https://old.reddit.com/r/emacs/comments/74jni0/fixing_ligature_indentation_proof_of_concept/][has received 'praise' from Emac's maintainer in the
    ...
    nt attempts to so egregiously abuse Emac's display engine that a skilled
    ...
    I'm speaking in jest - this project would complement, and hopefully inform,
    an extension of Emac's indentation engine and native ligature support. And
    in time, I hope to be that user.

- Similar work? :: nt is a novel extension of ~prettify-symbols~, which itself
                   has only a counterpart in ~vim-conceal~. Note nt is
